...
// Inngest Function to send Email on Tsk Creation 
    ...
    async ({event, step}) => {
        ...
        await sendEmail({
            ...
            body:  `<div style = "max-width: 600px;">
                    <h2>Hi ${task.assignee.name},${task.title}</h2>
                    ...
                    <p style="margin-top: 20px; font-size:14px; color:#6c75d;"> 
        ...
        if(new Date(task.due_date).toLocaleDateString() !== new DateString()){
            await step.sleepUntil('wait-for-the-due-date', new Date(task.due_date));

            await step.run('check-if-task-is-complete', async (params) => {
                const task = await prisma.task.findUnique({
                    where: {id: taskId},
                    include: {assignee: true, project: true}
